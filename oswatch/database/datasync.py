...
class SyncData:
    ...
    def sync_data(method=method,tablesrc=tablesrc,tabledst=tabledst,ownersrc=ownersrc,ownerdst=ownerdst,condition=condition):
        ...
        '''
        if len(column_pk):
            sql_diff_result=sqlquery.query_sql(sqlquery.format_text(sql_diff,column_pk[0],ownersrc+'.'+tablesrc,condition,ownerdst+'.'+tabledst,condition))
            ...
            if sql_diff_result:
                ...
                if method=='merge':
                    if len(column_pk):
                        ...
                        sql_merge=sqlquery.format_text(sql_merge,ownerdst+'.'+tabledst,ownersrc+'.'+tablesrc,'dst'+'.'+column_pk[0],'src'+'.'+column_pk[0],sql_merge_col1,condition,sql_merge_col2)
                    else:
                        print("empty list, no primary key on the table")
                    try:
                        # sqlquery.query_sql(sql_merge,isresult='false')
                        # sqlquery.query_sql('commit',isresult='false')
                        sqlquery.query_sql(sql_merge,isresult='false')
                        print(sql_merge)
                    ...
                    if len(column_pk):
                        sql_sync_delete=sqlquery.format_text(sql_sync_delete,ownerdst+'.'+tabledst,column_pk[0],column_pk[0],ownersrc+'.'+tablesrc,condition,ownerdst+'.'+tabledst,condition)
                        sql_sync_insert=sqlquery.format_text(sql_sync_insert,ownerdst+'.'+tabledst,ownersrc+'.'+tablesrc,condition,ownerdst+'.'+tabledst,condition)
                    else:
                        print("empty list, no primary key on the table")

                    print('The sql to sync data: '+'\n'+sql_sync_delete+'\n'+sql_sync_insert)
                    try:
                        sqlquery.query_sql(sql_sync_delete,isresult='false')
                        sqlquery.query_sql(sql_sync_insert,isresult='false')
                    except cx_Oracle.DatabaseError:
                        print("Sync data error!")
                    else:
                        print("Sync data successfully!")
