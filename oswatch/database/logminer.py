...
'''
archive_sql=syncdata.sqlformat(archive,logconf['start_time'],logconf['end_time'])
onlinelog=syncdata.sqlresult(log)
archivelog=syncdata.sqlresult(archive_sql)
...
'''
writer(logminer_shell,syncdata.sqlformat(logminer_shell_head,logconf['ORACLE_SID']))
writer(logminer_shell,syncdata.sqlformat(new_log,onlinelog[0]))
for i in range(1,len(onlinelog)):
	writer(logminer_shell,syncdata.sqlformat(add_log,onlinelog[i]))
for i in range(1,len(archivelog)):
	writer(logminer_shell,syncdata.sqlformat(add_log,archivelog[i]))
writer(logminer_shell,syncdata.sqlformat(logminer_shell_end,logminerdict,logconf['start_time'],logconf['end_time'],logminer_record,logconf['tablename'],logconf['schema']))